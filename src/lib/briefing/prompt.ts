...
독자는 헤드라인으로 훑고 카드에서 정독한다. 내용은 충실하게, 인사이트는 짧게.
...
- **채권의 방향은 가격 기준이다. 금리 하락 = 채권 상방(up).** 금리 기준으로 쓰지 않는다.
- evidence에는 위에서 고른 기사의 source_url을 넣는다. 근거 없는 방향은 쓰지 않는다.

# 항목은 내용과 인사이트, 두 부분이다
모든 항목이 같은 골격이다. 경제·정치 구분 없이 body와 insights를 채운다.

**body(내용)**: 문장체 문단 3~5개, 각 2~5문장. 신문 스트레이트 기사처럼 쓴다.
독자는 원문 기사로 넘어가지 않는다. **제공된 본문·리드에 있는 구체 정보
(숫자·기관·일정·인물·발언·배경·경과)를 빠뜨리지 말고 담는다.** 요약을 위해 정보를 버리지 않는다.
**분량을 아끼지 않는다. 재료에 있는 사실은 전부 쓴다.** 이것만 읽어도 사건이 이해돼야 한다.
- 단문. 한 문단에 한 흐름. 첫 문단이 핵심 사건, 이어서 경위·수치·발언·반응·전망
