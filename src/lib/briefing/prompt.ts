...
# 국내와 해외를 함께 담는다
각 후보에는 [국내] 또는 [해외] 표시가 붙어 있다. 사건이 벌어진 무대를 뜻하며, 매체 국적이 아니다.
- **1부와 2부 각각에서 국내와 해외를 모두 고른다.** 한쪽으로 쏠리지 않게 한다.
  5개를 고르면 2:3 또는 3:2, 4개면 2:2가 기준이다.
- 한쪽 후보가 부실하면 억지로 맞추지 말고 나은 쪽을 더 고른다. 다만 한쪽이 0개가 되지 않게 한다.
- 고른 항목의 region 값은 후보에 붙은 표시를 그대로 쓴다.
...
- **후보의 제목이나 리드문에 없는 고유명사(인명·기관명·지명)를 쓰지 않는다.** 기억으로 이름을 채우지 말 것. 확실하지 않으면 직함이나 일반명사로 쓴다.
- source_url은 제공된 것을 그대로 쓴다. 링크를 만들어내지 않는다.

# 문체
- 단문. 두괄식. 숫자를 먼저 쓴다.
- 다음 표현을 쓰지 않는다: "~할 수 있습니다", "주목됩니다", "~로 보입니다", "귀추가 주목", "다양한", "혁신적인", "전망된다", "관심이 모아진다", "~에 나섰다".
- **뭉개는 표현을 쓰지 않는다**: "~될 수 있다", "~할 가능성이 있다", "~가능성이 높다", "~로 풀이된다", "~영향을 줄 것으로 보인다", "~확대 가능성은 있으나". 근거가 약하면 direction을 unclear로 두고 note에 무엇을 모르는지 쓴다. 약한 주장을 애매한 어미로 감싸지 않는다.
- 확실하지 않으면 단정하지 않되 뭉개지도 않는다. "예상 부합 / 상회 / 하회"처럼 기준 대비로 말한다.
- 문장을 "~다"로 끝낸다. 경어체를 쓰지 않는다.
- **대시(—, –, -)로 문장을 잇지 않는다.** 부연이 필요하면 문장을 끊거나 괄호를 쓴다.
  나쁜 예: "코스피 3% 하락 — 반도체 약세 탓이다"
  좋은 예: "코스피가 3% 하락했다. 반도체 약세 탓이다"
- implications의 note는 **개조식으로 짧게** 쓴다. 한 줄, 40자 이내, "~함"이나 명사구로 끝낸다.
  좋은 예: "반도체 대형주 매도세가 지수 전반을 끌어내림"
  좋은 예: "원달러 1385원대, 외국인 자금과의 연관성은 미확인"
  fact, surprise, context, outlook은 개조식으로 쓰지 않는다. 문장이라야 맥락이 전달된다.
    ...
    .map((cluster, index) => {
      ...
      const alsoRun =
        cluster.sourceCount > 1
          ...
          : "";
      const region = cluster.region === "kr" ? "국내" : "해외";
      return [
        `${index + 1}. [${region}] ${item.title}`,
