...
- **알림:** Web Push (PWA, 6단계). 파이프라인 장애는 화면 배너로만 — 텔레그램 등 외부 채널은 만들지 않는다 (2026-08-27 확정).
...
3. **원본 저장.** 수집한 뉴스 메타데이터(제목·리드문·출처 링크)와 시세를 가공 전 raw 테이블에 저장. 브리핑 생성 실패 시 재시도 가능, 프롬프트 개선 후 과거 날짜 재생성 가능.
   - **기사 전문은 저장·표시하지 않는다** (저작권). 세부 내용 확인은 출처 링크로 원문 이동. 핵심 지표(금리·CPI 등)는 뉴스가 아니라 FRED/ECOS 원데이터를 1차 소스로 사용.
   - **본문 일시 수집 (2026-08-26 개정, CLAUDE.md §2-3).** 리드문 실측 결과 연합뉴스가 평균 75자(잘린 한 문장)라 해석 재료로 부족했다. 하루 선별 기사(~18건)에 한해 생성 직전에 본문을 가져와 프롬프트 재료로만 쓰고 버린다 (`src/lib/news/article.ts`: `<article>`/`<p>` 휴리스틱 + 문장부호 밀도 게이트, 동시 4건, 8초 타임아웃, 실패 시 리드 폴백). 입력 토큰 +16k, 비용 +$0.05 수준.
4. **헬스체크.** 수집 후 자가진단: 소스별 최소 건수 충족 여부. 실패는 `cron_runs`에 남고 브리핑 화면 배너가 알린다. 텔레그램 등 외부 알림 채널은 만들지 않는다 (2026-08-27 확정).
5. **멱등성 재시도.** 해당 날짜 브리핑이 이미 존재하면 스킵. 자동 재시도는 붙이지 않았다. 실패를 `cron_runs`에 남기고 화면이 알리는 쪽을 먼저 한다 (2026-08-26).
...
| 시각(KST) | 작업 |
|---|---|
| 06:30 | `/api/cron/daily` — 뉴스 수집 → 지표 수집 → 브리핑 생성·발행 |
| 장중 주기 | 시세 갱신, 급변 감지(±2%) |
| 이벤트 시각 +5분 | 이벤트 데이터 수집 → 즉시 호출 → 미니 브리핑 발행 |
| 일요일 | 딥다이브 배치 제출·발행 |
| KIS 동기화 | 장 마감 후 1회 + 앱 접속 시 |

**04:00에서 06:30으로 옮긴 이유 (2026-08-26).** 미 증시가 06:00 KST에 마감한다.
04:00에 돌리면 전날 미국장 마감 내용이 통째로 빠진다.

**한 라우트가 세 단계를 순서대로 부른다.** Vercel Cron은 체이닝을 못 하고,
Hobby 플랜은 크론 2개·하루 1회로 제한된다. 시각을 벌려 세 개를 거는 대신
`/api/cron/daily`가 순서를 보장한다. 수집이 실패하면 브리핑 생성을 건너뛴다.
어제 기사로 오늘 브리핑을 쓰느니 하루 비우는 쪽이 낫다.

**지표 수집을 따로 부른다.** `generateBriefing`이 안에서 `collectMacro`를 부르지만
이미 오늘 브리핑이 있으면 그 전에 빠져나간다. 함의 채점은 매일의 `raw_market`이
빠짐없이 있어야 성립하므로 브리핑 성패와 무관하게 지표를 남긴다.

**Vercel Cron 인증.** `CRON_SECRET` 환경변수를 등록하면 Vercel이 호출 시
`Authorization: Bearer $CRON_SECRET`을 자동으로 붙인다. 라우트는 이 헤더만 본다.

**실행 기록은 `cron_runs`.** 단계마다 성공·실패를 남기고 브리핑 화면이 읽는다
(0011). 자동화의 위험은 실패가 아니라 조용한 실패다. 놓친 날의 기사는
RSS가 다시 주지 않으므로 복구가 없다.
